...
"""! Module for a EinstAlertHandler which listens and responds to Einstein probe alerts from GCN
"""

class EinstAlertHandler(AlertHandler):
    """! EinstAlertHandler. Generates targets from a einsteing event notice
    """
    def __init__(self, event, buffer, logger):
        """! Construct a EinstAlertHandler instance
        """
    ...
    def _alert_listener(self):
        """! Alert listener for the EinstAlertHandler. Subscribes to einstein notices
        """
    ...
    def handle_alert(self, data):
        ...
        event_name = f'{data['instrument']}{data['id']}'

        self.logger.info(f"Handling an Alert: {event_name}")

        # Write json for records        
        with open(f"{data_path}/{event_name}.json", "w") as outfile:
            outfile.write(data)
        ...
        try:
            ra = data['ra']
            dec = data['dec']
            error = data['ra_dec_error'] + error_buff
        except:
            self.logger.error("ra, dec, or error element not found in the json.")
            return
        ...
        # Notify Scheduler
        self.schedule_buffer.name = event_name

...
parser.add_argument("ra", type=float, help="right ascension <float>")
parser.add_argument("dec", type=float, help="declination <float>")
...
r = session.post(
    f"{protocol}://{telescope['ip']}:{telescope['port']}/telescope_controller/behavior/mount/point",
    data=json.dumps(settings), headers=headers)
util.handle_telescope_request(r, telescope)
print(f"moved telescope to ra: {args.ra}, dec: {args.dec}")
